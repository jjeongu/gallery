<%@ page contentType="text/html; charset=UTF-8" %>
<%@ taglib prefix="c" uri="jakarta.tags.core"%>
<%@ taglib prefix="fmt" uri="jakarta.tags.fmt"%>

<div class="container-fluid p-0" style="background: lightgray;">
	<div style="background-color: black; color: white; height: 300px;">
	<ul class="nav justify-content-center p-3">
	 	<li class="nav-item"><span>&nbsp;&middot;&nbsp;</span></li>
        <li class="nav-item"><a class="text-reset" href="#">서울 마포구 월드컵북로 21 풍성빌딩 c호</a></li>
        <li class="nav-item"><span>&nbsp;&middot;&nbsp;</span></li>
        <li class="nav-item"><a class="text-reset" href="#">개인정보취급방침</a></li>
        <li class="nav-item"><span>&nbsp;&middot;&nbsp;</span></li>
        <li class="nav-item"><a class="text-reset" href="#">신고하기</a></li>
        <li class="nav-item"><span>&nbsp;&nbsp;© Pat&Mat gallery.</span></li>
    </ul>
	</div>
</div>


