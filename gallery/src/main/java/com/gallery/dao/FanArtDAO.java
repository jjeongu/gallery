...
public class FanArtDAO {
	...
	public List<FanArtDTO> list(int offset, int size, String schType, String kwd) {
		...
		try {
			...
			if (schType.equals("artist"))
				schType = "art.name";
			if (schType.equals("name"))
				schType = "m.name";

			sb.append(" select num, f.member_id, m.name, notice, subject, content, ");
			sb.append(" hitcount, to_char(f.reg_date, 'YYYY-MM-DD HH24:mm:dd') reg_date, ");
			sb.append(" img, artist, art.name artname from fanArt f ");
			sb.append(" join member1 m on f.member_id=m.member_id ");
			sb.append(" left outer join member1 art on f.artist=art.member_id where notice = 0 ");
	...
	public FanArtDTO findByPrev(int notice, long num, String schType, String kwd) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuilder sb = new StringBuilder();
		FanArtDTO dto = null;
		...
		try {
			...
			sb.append("select num, subject from fanArt f join member1 m on f.member_id=m.member_id ");
			sb.append(" left outer join member1 art on f.artist=art.member_id ");
			sb.append(" where notice = ? and num > ? ");
			if (kwd != null && kwd.length() != 0) {
				if (schType.equals("reg_date")) {
					kwd = kwd.replaceAll("(\\-|\\/|\\.)", "");
					sb.append(" AND TO_CHAR(f.reg_date, 'YYYYMMDD') = ? ");
				} else {
					sb.append(" and instr("+schType+", ?)");
				}
	...
	public FanArtDTO findByNext(int notice, long num,  String schType, String kwd) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		StringBuilder sb = new StringBuilder();
		FanArtDTO dto = null;
